...
* 说明：
  ...
  * 下滑用于结束当前通话控制；未锁定时取消本次通话唤起，已锁定时立即停止收音并异步发送最后一段
...
* 在左键长按过程中，支持以下状态控制：
  ...
  * 上滑：锁定当前通话状态，进入持续通话模式；进入锁定后，每次停顿都应自动提交当前语音段并继续监听

  * 下滑：未锁定时取消本次通话唤起；已锁定时立即结束当前通话，最后一段语音异步送入后续处理链路
...
* 以左键长按为主，作为主要表达方式。用户可在唤起后通过上滑锁定通话；未锁定时下滑取消本次输入，锁定后下滑结束当前通话。
...
* 用户通过左键长按唤起通话能力后，可通过不同手势控制当前状态：
  ...
  * 上滑锁定：进入持续通话模式，适用于连续表达、补充背景或多轮追问；锁定后应按语音停顿自动分段发送

  * 下滑结束：未锁定时放弃本次输入，不进入后续处理流程；已锁定时立即停止收音，并把最后一段话异步发送、异步接收响应

* 锁定通话模式下，第一段语音通过正式输入提交通道创建任务；后续段优先追加到当前仍活跃的同一任务。若当前任务已结束，下一段再开启同一语音会话下的新任务。
